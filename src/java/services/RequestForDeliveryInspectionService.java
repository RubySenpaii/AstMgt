...
public class RequestForDeliveryInspectionService {
    ...
    public int AddRequestForDeliveryInspection(RequestForDeliveryInspection deliveryRequest) {
        try {
            ...
            String query = "INSERT INTO RequestForDeliveryInspection (" + RequestForDeliveryInspection.COLUMN_CREATED_BY + ", "
                    ...
                    + RequestForDeliveryInspection.COLUMN_ASSIGNED_TO + ", " + RequestForDeliveryInspection.COLUMN_IS_COMPLETED + ") "
                    + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            ps.setInt(11, deliveryRequest.IsCompleted);
        ...
        try {
            ...
            if (deliveryReceipt.ApprovedBy > 0) {
                query = "UPDATE RequestForDeliveryInspection SET " + RequestForDeliveryInspection.COLUMN_CREATED_BY + " = ?, "
                        ...
                        + RequestForDeliveryInspection.COLUMN_APPROVED_BY + " = ?, " + RequestForDeliveryInspection.COLUMN_IS_COMPLETED + " = ? "
                ...
                ps.setInt(11, deliveryReceipt.IsCompleted);
                ps.setInt(12, deliveryReceipt.DeliveryInspectionId);
            } else {
                query = "UPDATE RequestForDeliveryInspection SET " + RequestForDeliveryInspection.COLUMN_CREATED_BY + " = ?, "
                        + RequestForDeliveryInspection.COLUMN_CREATED_DATE + " = ?, " + RequestForDeliveryInspection.COLUMN_DELIVERY_RECEIPT + " = ?, "
                        + RequestForDeliveryInspection.COLUMN_FROM_BIDDING + " = ?, " + RequestForDeliveryInspection.COLUMN_INVOICE + " = ?, "
    ...
    public ArrayList<RequestForDeliveryInspection> GetPendingRfi() {
        try {
            DBConnectionFactory db = DBConnectionFactory.getInstance();
            Connection con = db.getConnection();

            String query = "SELECT * FROM RequestForDeliveryInspection WHERE IsCompleted = 0";
            PreparedStatement ps = con.prepareStatement(query);
    ...
    public RequestForDeliveryInspection GetRequestForInspectionByPurchaseOrder(int purchaseOrder) {
        try {
            DBConnectionFactory db = DBConnectionFactory.getInstance();
            Connection con = db.getConnection();

            String query = "SELECT * FROM RequestForDeliveryInspection "
                    + "WHERE " + RequestForDeliveryInspection.COLUMN_PURCHASE_ORDER_ID + " = ?";
            PreparedStatement ps = con.prepareStatement(query);
            ps.setInt(1, purchaseOrder);
    ...
    private ArrayList<RequestForDeliveryInspection> getResult(ResultSet rs) throws SQLException {
        ...
        while (rs.next()) {
            ...
            deliveryRequest.IsCompleted = rs.getInt(RequestForDeliveryInspection.COLUMN_IS_COMPLETED);
